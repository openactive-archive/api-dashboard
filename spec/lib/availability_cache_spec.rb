...
describe AvailabilityCache do
  ...
  describe ".update" do
    ...
    it "doesn't store availability if there were issues making requests" do
      WebMock.stub_request(:get, "https://activenewham-openactive.herokuapp.com").to_raise(StandardError)
      WebMock.disable_net_connect!
      AvailabilityCache.update
      availability = JSON.parse(Redis.current.get('availability'))
      expect(availability).to eql({})
    end
